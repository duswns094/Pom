    ...
    def get(self, request, *args, **kwargs):
        ...
        if soso_rec.exists():
            ...
            if dislike_rec.exists():
                dislike_rec.delete()
                article.dislike -= 1
            SosoRec(user=user,article=article).save()
            article.soso += 1
            article.save()
        return super(SosoView, self).get(request, *args, **kwargs)

@method_decorator(login_required, 'get')
class SosoListView(ListView):
    model = Article
    context_object_name = 'article_list'
    template_name = 'sosoapp/list.html'
    paginate_by = 5

    def get_queryset(self):
        articles = SosoRec.objects.filter(user=self.request.user).values_list('article')
